...
public class ScheduleUpdatePriceServiceImpl {
    ...
    public ScheduleUpdatePriceServiceImpl(PriceRepository priceRepository, PriceServiceImpl priceService, PriceCacheServiceImpl priceCacheServiceImpl) {
    ...
    public List<Price> scheduleUpdateBestPrice() {
        BinancePrice[] binancePrices = priceService.fetchBinancePrices();
        ...
        HoubiPrice.HoubiTicker[] houbiPrices = priceService.fetchHuobiPrices();
        HoubiPrice.HoubiTicker[] filteredHoubiPrice = Arrays.stream(houbiPrices)
                .filter(price -> VALID_CRYPTO_PAIRS.contains(price.getSymbol().toUpperCase()))
                .toArray(HoubiPrice.HoubiTicker[]::new);

        Map<String, AggregatedPrice> bestPrices = priceService.getBestPrices(filteredBinancePrices, filteredHoubiPrice);
